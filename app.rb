...
get "/" do
  ...
  @forecast = HTTParty.get(url).parsed_response.to_hash
    ...
    @current = ["Temperature: #{@forecast["current"]["temp"]} degrees", "Sky: #{@forecast["current"]["weather"][0]["description"]}"]
    puts @current
    puts "Weekly Forecast:"
    extended = []
    day_number = 1
    for day in @forecast["daily"]
        extended << "#{day_number} Day: Max temperature of #{day["temp"]["max"]} F and a Min temperature of #{day["temp"]["min"]} F with #{day["weather"][0]["description"]}"
